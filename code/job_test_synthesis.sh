...
## --time=일-시간:분:초
# sbatch --gres=gpu:4 --time=48:00:00 ./job_test_synthesis.sh
...
bash run_NL_PL.sh 0 desc python program codet5 eval

# slurm-297192.out : python run.py --do_train ... 과 python run.py --do_test ... 를 돌린 결과 / train batch size 16, eval batch size 16 으로 설정함.
# slurm-297349.out : bash run_NL_PL.sh 0 desc python program codet5 eval 를 돌린 결과 / eval batch size 16 으로 설정함. / evaluator.py랑 calc_code_bleu.py 실행시 arg 이상하게들어간듯. 여기서 에러발생